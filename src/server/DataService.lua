...
--------------------------------------------------------------------------------
-- ลบ (DEBUG เท่านั้น — ใช้ทดสอบ flow "ผู้เล่นใหม่" ซ้ำได้โดยไม่ต้องใช้ account อื่น)
--------------------------------------------------------------------------------

-- ⚠️⚠️ ลบข้อมูลออกจาก DataStore แบบถาวร กู้คืนไม่ได้ — ต่างจากทุก path อื่นในไฟล์นี้
-- ที่ใช้ UpdateAsync เขียนทับด้วยของใหม่เสมอ (ไม่เคยใช้ SetAsync) ตัวนี้ตั้งใจใช้ RemoveAsync
-- เพราะ "เขียนทับด้วยข้อมูลว่างเปล่า" ไม่พอ — key ต้องกลับไปเป็น **ไม่เคยมี** (`stored == nil`)
-- เท่านั้นถึงจะทำให้ loadAsync ตัดสิน isNew = true อีกครั้งตอนเข้าเกมครั้งถัดไป
--
-- ⚠️ เรียกได้เฉพาะผู้เล่นที่ยังออนไลน์อยู่บนเซิร์ฟเวอร์นี้ (เช็คจาก cache) เพราะเหตุผลสำคัญกว่า
-- แค่ sanity check: **ต้องเคลียร์ cache[userId] ทันทีในฟังก์ชันนี้เอง** ไม่งั้นเซสชันที่ยังเล่นอยู่
-- จะมี data เก่าค้างในหน่วยความจำ แล้ว autosave/เซฟตอนออกเกมรอบถัดไปจะเขียนของเก่านั้น
-- กลับเข้า DataStore ทันที = ลบไปแล้วก็เหมือนไม่ได้ลบ ผู้เรียก (EggService) จึงต้อง**เตะผู้เล่น
-- ออกทันที**หลังเรียกฟังก์ชันนี้สำเร็จด้วย ไม่ใช่ปล่อยให้เล่นต่อในเซสชันเดิม
function DataService.wipeAsync(userId: number): (boolean, string?)
	if not cache[userId] then
		return false, "ไม่มีข้อมูลผู้เล่นนี้ในหน่วยความจำของเซิร์ฟเวอร์นี้ (ต้องออนไลน์อยู่ตอนเรียก)"
	end

	local key = DataService.keyFor(userId)
	local ok, err = DataService.retryAsync(`ลบข้อมูลของ {userId}`, function()
	...
	print(`[DataService] ลบข้อมูลของ {userId} ออกจาก DataStore แล้ว — เข้าเกมครั้งถัดไปจะนับเป็นผู้เล่นใหม่`)
	return true, nil
end
