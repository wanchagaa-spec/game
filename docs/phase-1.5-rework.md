...
| `EggSlot` 4 ช่องตาม `Config.Farm.EGG_SLOTS_PER_PLAYER` | สวนฟัก `Config.Balance.Hatchery.MAX_SLOTS` = 50 ช่อง | ดีไซน์ใหม่ |
...
| `Config.Farm.EGG_SLOTS_PER_PLAYER` = 4 | **ตกยุค** — สวนฟักคือ `Config.Balance.Hatchery.MAX_SLOTS` = 50 | ✅ ลบทิ้งแล้ว · ภายหลังแยกเป็น `MAX_SLOTS` (ช่องฟัก) กับ `BAG_CAPACITY` (กระเป๋าไข่) |
...
| `Config.Balance.NewPlayer.startingEggs` | **ยังใช้ได้** แต่เป็นแค่ "คำสั่งแจก" | ✅ `onPlayerAdded` วนเรียก `grantEgg()` ทีละฟอง แต่ละฟองจึงสุ่มน้ำหนักของตัวเอง |
...
- [x] validate `eggId` ครบ 8 กรณี — **ยืนยันแล้วด้วยเทสต์อัตโนมัติ**
      `tools/check-egg-placement-validation.py` (ยิงเข้า callback ของ
      `PlaceEggInHatcheryRequest` ตัวจริงที่ผูกไว้ใน `EggService.start()` ไม่ใช่เรียก
      `EggService.placeEgg()` ลอย ๆ — ถ้าใครแก้ remote handler ทีหลังแล้วลืมเรียก
      validate เทสต์นี้จะจับได้)

  > ⚠️ ของเดิมใช้ `heldIndex` (ตำแหน่งในอาเรย์) ก่อน Phase 2A เปลี่ยนกลไกเป็น
  > `eggId` (id ประจำฟอง) ทั้งหมดแล้ว เช็คลิสต์นี้เขียนใหม่ตามโครงปัจจุบัน —
  > ของเดิม ("validate `heldIndex` ครบ 5 กรณี (0, 51, ทศนิยม, ช่องว่าง, ไม่ใช่ number)")
  > ยังหาดูได้ใน git history ของไฟล์นี้ ไม่ได้ลบทิ้งเงียบ ๆ

  | # | payload | ผลที่ควรได้ |
  |---|---|---|
  | 1 | `eggId = 0` | ปฏิเสธ |
  | 2 | `eggId` ไม่เคยมีอยู่ในกระเป๋าเลย (999999) | ปฏิเสธ |
  | 3 | `eggId` เป็นทศนิยม (3.5) | ปฏิเสธ |
  | 4 | `eggId` เป็น string ("abc") | ปฏิเสธ |
  | 5 | `eggId = nil` (ไม่ส่งอะไรเลย) | ปฏิเสธ |
  | 6 | `eggId` ติดลบ (-5) | ปฏิเสธ |
  | 7 | `eggId` เคยมีจริงแต่ถูกใช้ไปแล้ว (ใช้ซ้ำ) | ปฏิเสธ — id ไม่ถูก reuse (`nextEggId` เดินหน้าเรื่อย ๆ ตามที่ตกลงไว้ Phase 2A) |
  | 8 | `eggId` ของผู้เล่นอีกคน (คนละ session) | ปฏิเสธ — กัน exploit ข้ามบัญชี |

  ทุกกรณีปฏิเสธเงียบ ๆ ไม่ error ไม่ crash เซิร์ฟเวอร์ (มีเคส "สุขภาพดี" ควบคู่ไปด้วย
  ยืนยันว่า `eggId` ที่ถูกต้องยังวางสำเร็จจริง กันไม่ให้เช็ค "ข้อมูลไม่เปลี่ยน" ข้างบน
  กลายเป็น false negative)
